...
async function setNoteAction(formData: FormData) {
  ...
  if (error) redirect('/living/app/bill?error=' + encodeURIComponent(error.message))
  redirect('/living/app/bill?notice=' + encodeURIComponent('Note saved.'))
}
...
async function raiseDisputeAction(formData: FormData) {
  ...
  if (!reason) redirect('/living/app/bill?error=' + encodeURIComponent('Add a short reason.'))
