...
async function uploadFlatsAction(formData: FormData) {
  ...
  if (!file || file.size === 0) redirect('/living/app/setup?error=' + encodeURIComponent('Choose a file first.'))

  const { rows, errors } = await parseFlatTemplate(await file.arrayBuffer())
  if (rows.length === 0) {
    redirect('/living/app/setup?error=' + encodeURIComponent(errors[0] ?? 'No flats found in that file.'))
  }

  const existing = await getFlats(supabase, apartment.id)
  if (existing.length + rows.length > apartment.flat_count) {
    redirect(
      '/living/app/setup?error=' +
        encodeURIComponent(
  ...
  if (error) redirect('/living/app/setup?error=' + encodeURIComponent(error.message))

  const notice = errors.length > 0 ? `Added ${rows.length} flats. ${errors.length} row(s) skipped — see below.` : `Added ${rows.length} flats.`
  redirect('/living/app/setup?notice=' + encodeURIComponent(notice))
}
...
async function addFlatAction(formData: FormData) {
  ...
  if (existing.length >= apartment.flat_count) {
    redirect(
      '/living/app/setup?error=' +
        encodeURIComponent(`This Apartment is already at its configured ${apartment.flat_count}-flat count — update it below if that's wrong.`)
    )
  }
  ...
  if (error) redirect('/living/app/setup?error=' + encodeURIComponent(error.message))
  redirect('/living/app/setup?notice=' + encodeURIComponent('Flat added.'))
}
...
async function updateFlatsAction(formData: FormData) {
  ...
  for (const flat of flats) {
    ...
    const { error } = await supabase
      ...
      .eq('id', flat.id)
    if (error) redirect('/living/app/setup?error=' + encodeURIComponent(`Flat ${flat.flat_no}: ${error.message}`))
  }
  redirect('/living/app/setup?notice=' + encodeURIComponent('Flats updated.'))
}
...
async function updateFlatCountAction(formData: FormData) {
  ...
  if (flatCount < 1 || flatCount > PLATFORM_MAX_FLATS) {
    redirect('/living/app/setup?error=' + encodeURIComponent(`Flat count must be between 1 and ${PLATFORM_MAX_FLATS}.`))
  }

  const existing = await getFlats(supabase, apartment.id)
  if (flatCount < existing.length) {
    redirect(
      '/living/app/setup?error=' + encodeURIComponent(`Can't set the flat count below ${existing.length} — that many flats already exist.`)
    )
  }

  const { error } = await supabase.from('living_apartments').update({ flat_count: flatCount }).eq('id', apartment.id)
  if (error) redirect('/living/app/setup?error=' + encodeURIComponent(error.message))
  redirect('/living/app/setup?notice=' + encodeURIComponent('Flat count updated.'))
}
...
async function updateSharedCostDivisorAction(formData: FormData) {
  ...
  if (divisor !== null && (!Number.isFinite(divisor) || divisor <= 0)) {
    redirect('/living/app/setup?error=' + encodeURIComponent('Divisor must be a positive number, or left blank to use the actual flat count.'))
  }

  const { error } = await supabase.from('living_apartments').update({ shared_cost_divisor: divisor }).eq('id', apartment.id)
  if (error) redirect('/living/app/setup?error=' + encodeURIComponent(error.message))
  redirect('/living/app/setup?notice=' + encodeURIComponent('Split divisor updated.'))
}
...
async function approveClaimAction(formData: FormData) {
  ...
  if (error) redirect('/living/app/setup?error=' + encodeURIComponent(error.message))
  redirect('/living/app/setup?notice=' + encodeURIComponent('Request approved.'))
}
  ...
  return (
    <>
      ...
      <div className={homeStyles.section}>
        ...
        <div className={theme.card} style={{ marginTop: '0.75rem' }}>
          ...
          <p className={theme.muted} style={{ marginTop: '0.75rem' }}>
            Share override only matters when this Apartment&rsquo;s maintenance split is set to weighted, and only if you don&rsquo;t
            want to rely on sq ft for that flat. &ldquo;Shared/common meter&rdquo; is for a row that isn&rsquo;t a real resident (e.g.
            a building-wide common meter) — it never gets a personal bill, and its own water charge auto-fills the Common Water Bill
            line item on Maintenance instead. &ldquo;Merge water into&rdquo; is for a flat with a second meter, same owner as another
            unit — its water charge adds into the flat you pick, and it drops off the Bills page on its own.
          </p>
        </div>
      </div>

      <div className={homeStyles.section}>
        <h2 className={homeStyles.sectionTitle}>Common cost split</h2>
        <div className={theme.card}>
          <p className={theme.muted} style={{ marginBottom: '1rem' }}>
            Equal-split common maintenance and the shared water-supply pool (tankers/Majeera) both divide by the actual number of
            flats by default. Override that here if it shouldn&rsquo;t be — e.g. a flat with two water meters is still one billable
            unit, or not every row above is really a separate one.
          </p>
          ...
          <p className={theme.muted} style={{ marginTop: '0.5rem' }}>
            ...
            {apartment.shared_cost_divisor ? ' (override)' : ` (actual flat count — leave blank to keep this automatic)`}. Leave blank
            to go back to automatic. When overridden, per-flat shares won&rsquo;t necessarily add back up to the grand total — that&rsquo;s
            expected.
          </p>
        </div>
      </div>
      ...
      <div className={homeStyles.section}>
        <h2 className={homeStyles.sectionTitle}>Pending flat requests ({claims.length})</h2>
