...
async function saveSlabAction(formData: FormData) {
  ...
  if (error) redirect('/living/app/settings/slabs?error=' + encodeURIComponent(error.message))
    ...
    <>
      ...
      <div className={theme.card} style={{ marginBottom: '1.5rem' }}>
        <form action={saveSlabAction}>
          <div className={theme.field}>
            ...
            <p className={theme.muted} style={{ marginTop: '0.3rem' }}>Past bills are never rewritten by this change.</p>
          </div>
          ...
          <div className={theme.field}>
            ...
            {suggestedRate !== null && (
              <p className={theme.muted} style={{ marginTop: '0.3rem' }}>
                Suggested from this month&rsquo;s tanker/Majeera cost: {formatCurrency(suggestedRate)} per 1,000L — pre-filled above,
                edit freely.
              </p>
            )}
          </div>
          ...
          <div className={homeStyles.grid} style={{ marginBottom: '1rem' }}>
            ...
            <div>
              ...
              <input name="tier2_mult" type="number" step="0.01" className={theme.input} style={{ marginTop: '0.4rem' }} defaultValue={tier2?.rate_multiplier ?? 1.2} />
            </div>
            <div>
          ...
          <div className={homeStyles.grid} style={{ marginBottom: '1rem' }}>
            <div>
              ...
              <input id="grace_period_days" name="grace_period_days" type="number" className={theme.input} defaultValue={latest?.grace_period_days ?? 15} />
            </div>
            <div>
              <label className={theme.label} htmlFor="escalation_cadence">
                Escalation cadence
              </label>
              <select id="escalation_cadence" name="escalation_cadence" className={theme.select} defaultValue={latest?.escalation_cadence ?? 'monthly'}>
      ...
      {configs && configs.length > 0 && (
        <div className={theme.card}>
          <h2 className={homeStyles.sectionTitle}>History</h2>
